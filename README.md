...
## Tuning
The smoothing and brightness mapping constants live in `LightSensorBridge`. Defaults map lux into a brightness window of **-76 .. 15**.  
- Override at runtime via env/system props (for Studio add them in the run config): `LIGHT_LUX_MIN`, `LIGHT_LUX_MAX`, `LIGHT_BRIGHT_MIN`, `LIGHT_BRIGHT_MAX`, `LIGHT_BRIGHT_DELTA_MIN`, `LIGHT_FORCE_SEND_MS`, `LIGHT_UPDATE_INTERVAL_MS`, `LIGHT_SKIP_SMOOTHING`.  
- By default the bridge skips its own smoothing (`LIGHT_SKIP_SMOOTHING=true`) to avoid double-filtering when the ESP firmware already smooths readings. Flip it to `false` if you post raw lux.  
